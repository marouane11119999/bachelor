...
def fast_fd_counts(
    ...
    i = 1
    for fd in fds:
        print('initialized ',i, ' out of',len(fds),flush=True)
        d[fd.lhs][fd.rhs] = {}
        i = i+1
    i=1
    for fd in fds:
        print('Processed fd',i, ' out of',len(fds),flush=True)
...
def mine_fds(df_clean_iterative: pd.DataFrame, df_ground_truth: pd.DataFrame) -> List[FDTuple]:
    ...
    try:
        subprocess.run(["java", "-Xmx8G","-jar", binary, dirty_path, clean_path, fd_path],
